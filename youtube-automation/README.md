# 유튜브 자동화 파이프라인 (무료 도구 버전)

두 가지를 만들 수 있습니다.

- **롱폼 영상** (이 문서): 트렌드 리서치 → 스크립트 → 나레이션 → 영상 합성
- **쇼핑 쇼츠** ([SHORTS.md](SHORTS.md)): 쿠팡 파트너스 상품으로 세로 쇼츠 제작
...
- **구간별로 관련 영상만 씁니다.** `[SECTION: ... | keywords]` 태그의 키워드로 받은
  영상을 그 구간에만 붙입니다. 말하는 내용과 화면이 따로 노는 걸 줄여줍니다.
- **컷을 짧게 끊습니다.** 기본 4초이고 `--cut-seconds`로 바꿉니다. 한 장면이 오래
  가만히 있는 게 자동 생성 영상이 지루해 보이는 가장 큰 원인이라 이걸 나눕니다.
  같은 영상을 다시 쓸 때는 다른 구간을 잘라 씁니다.
- **컷 사이를 부드럽게 넘깁니다.**
- **아주 느린 확대를 넣습니다.** 화면이 멈춰 있지 않게 해줍니다.
  렌더링이 느려지므로 급할 땐 `--no-zoom`으로 끄세요.
- **배경음악을 깝니다.** `assets/music` 폴더에 음악을 넣어두면 자동으로 씁니다.
  넣는 방법은 그 폴더의 README를 보세요. `--no-music`으로 끌 수 있습니다.

자막은 같은 이름의 `.srt`가 있으면 그 타이밍을 쓰고, 없으면 문장 균등 분할로 대체합니다.
한글 폰트는 자동으로 찾습니다. 윈도우는 맑은 고딕, 맥은 애플 고딕을 씁니다.
`--font`로 직접 지정할 수 있고, `--no-captions`를 붙이면 자막 없이 뽑습니다.

### 결과가 밋밋할 때 손볼 것

- 컷을 더 빠르게: `--cut-seconds 2.5`
- 배경 영상 가짓수 늘리기: `main.py --broll-count 8` (같은 장면 반복이 줄어듭니다)
- 섹션 키워드를 더 구체적으로. `space`보다 `deep space nebula timelapse`가 훨씬 낫습니다.
- 배경음악 넣기 (`assets/music`)
